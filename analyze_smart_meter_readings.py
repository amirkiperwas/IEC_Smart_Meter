...
PLANS.append({'plan name': 'PazGaz day discount', 'percent discount': 15, 'from weekday': 0, 'to weekday': 6,
...
PLANS.append({'plan name': 'PazGaz Weekend discount', 'percent discount': 10, 'from weekday': 4, 'to weekday': 5,
              'from hour': '00:00', 'to hour': '23:59'})
PLANS.append({'plan name': 'PazGaz Night discount', 'percent discount': 15, 'from weekday': 0, 'to weekday': 6,
              'from hour': '23:00', 'to hour': '06:59'})
PLANS.append({'plan name': 'Electra flat discount', 'percent discount': 7, 'from weekday': 0, 'to weekday': 6,
              'from hour': '00:00', 'to hour': '23:59'})
PLANS.append({'plan name': 'Electra Hi-Tec discount', 'percent discount': 10, 'from weekday': 0, 'to weekday': 6,
...
def print_df(df_data):
    for ind in df_data.index:
        print(df_data['Interval starting date'][ind], df_data['day_of_week'][ind], df_data['Interval starting time'][ind],
...
if __name__ == '__main__':
    ...
    print('total without dicount: ', total_no_discount, ' NIS')
    for plan in PLANS:
    ...
    if not no_graph_flag:
        ...
        data_pivot = pd.pivot_table(data_pivot, index='Interval starting time', values='Consumption, kWh', columns='day_of_week', aggfunc='sum')
