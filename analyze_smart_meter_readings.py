...
PLANS.append({'plan name': 'Cellcom Work from home discount', 'percent discount': 15, 'from weekday': 6, 'to weekday': 3,
...
def main():
    ...
    for plan in PLANS:
        ...
        results.append({'plan name': plan_name, 'total with discount': new_total, 'savings': total_no_discount-new_total , 'savings %': saving_pecent})
    ...
    print('\nBest plan for you: plan "', max_savings_plan['plan name'], '" would have saved you', "{:.2f}".format(max_savings_plan['savings']), ' NIS','which is', "{:.2f}".format(max_savings_plan['savings %']), '%')
